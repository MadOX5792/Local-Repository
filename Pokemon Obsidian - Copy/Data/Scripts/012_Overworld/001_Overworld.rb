...
EventHandlers.add(:on_step_taken, :grass_rustling,
  proc { |event|
    ...
    # Figure out which map and tile the event is now on
    map_id = (event.respond_to?(:map) && event.map) ? event.map.map_id : event.map_id
    x = event.x
    y = event.y

    # Get the terrain tag for JUST this tile
    terrain_tag = $map_factory.getTerrainTagFromCoords(map_id, x, y, true)
    next if !terrain_tag || !terrain_tag.shows_grass_rustle

    # Play the grass rustle animation on that tile only
